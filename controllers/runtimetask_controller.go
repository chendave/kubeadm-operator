...
func (r *RuntimeTaskReconciler) Reconcile(ctx context.Context, req ctrl.Request) (_ ctrl.Result, rerr error) {
...
func (r *RuntimeTaskReconciler) reconcileNormal(executionMode operatorv1.OperationExecutionMode, task *operatorv1.RuntimeTask, log logr.Logger) error {
	...
	log.WithValues("Get into the command...", task.Status.CurrentCommand).Info("running command")
	if executionMode == operatorv1.OperationExecutionModeDryRun {
		...
		log.Info("Dry run... just return from here")
